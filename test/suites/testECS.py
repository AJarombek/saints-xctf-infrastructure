...
class TestECS(unittest.TestCase):
    ...
    def test_target_groups_exist(self):
        ...
        self.assertIn(self.ui_tg, tg_names, f"Target group '{self.ui_tg}' does not exist.")
        self.assertIn(self.api_tg, tg_names, f"Target group '{self.api_tg}' does not exist.")
    ...
    def test_ecs_cluster_active(self):
        ...
        self.assertEqual(clusters[0]["status"], "ACTIVE", f"ECS cluster '{self.cluster_name}' is not ACTIVE.")

    def test_ecs_service_running_tasks(self):
        for service in [self.ui_service, self.api_service]:
            response = self.ecs.describe_services(cluster=self.cluster_name, services=[service])
            services = response["services"]
            self.assertTrue(services, f"ECS service '{service}' not found.")
            running_count = services[0]["runningCount"]
            self.assertGreater(running_count, 0, f"ECS service '{service}' has no running tasks.")
    ...
    def test_target_groups_have_healthy_targets(self):
        for tg_name in [self.ui_tg, self.api_tg]:
            ...
            health = self.elbv2.describe_target_health(TargetGroupArn=tg_arn)["TargetHealthDescriptions"]
    ...
    def test_ecs_service_desired_count(self):
        for service in [self.ui_service, self.api_service]:
            ...
            self.assertGreaterEqual(desired_count, 1, f"ECS service '{service}' desired count is less than 1.")
